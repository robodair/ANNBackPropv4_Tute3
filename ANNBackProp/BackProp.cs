...
{ 
    public class NeuralNetwork
    {
        ...
        public void train(double[][] trainData, double [][] testData, int maxEpochs, double learnRate, string logFileName, Chart chart, ProgressBar progressBar)
        {
            // train a back-prop style NN classifier using learning rate and momentum
            // weight decay reduces the magnitude of a weight value over time unless that value
            // is constantly increased
            ...
            using (StreamWriter writer = new StreamWriter(logFileName))
            {
                ...
                {
                    percentageComplete = (maxEpochs / 100);
                    progressBar.Increment(percentageComplete);

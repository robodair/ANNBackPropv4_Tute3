...
namespace ANNShell
{
    ...
    public bool setRow(int rowNum, double[] rowdata)
        ...
        public void normalize(int firstCol, int lastCol)
        {
            // normalize specified cols by computing (x - mean) / sd for each value
            if (normMin == null) normMin = new double[columns];
            if (normMax == null) normMax = new double[columns];
            if (normMul == null) normMul = new double[columns];
